...
/* ------------------------------------------------------------------ */
/*  Animated protocol flow diagram (SVG)                               */
/* ------------------------------------------------------------------ */
function ProtocolFlowDiagram() {
  const { ref, inView } = useInView({ threshold: 0.2 })

  const steps = [
    { label: 'User\nDeposits USDC', icon: '💰', color: '#0EA5E9' },
    { label: 'AI Keeper\nScores Protocols', icon: '🤖', color: '#8B5CF6' },
    { label: 'Guardrails\nEnforce Limits', icon: '🛡️', color: '#F59E0B' },
    { label: 'Capital Routes\nto Best Yield', icon: '📈', color: '#10B981' },
  ]
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 relative">
        {steps.map((step, i) => (
          <div key={i} className="relative">
            <div
              className="p-5 rounded-2xl border border-white/[0.06] bg-white/[0.02] text-center transition-all duration-700 hover:border-white/[0.12] hover:bg-white/[0.04]"
              style={{
                opacity: inView ? 1 : 0,
              ...
              <div className="hidden md:flex absolute top-1/2 -right-2 -translate-y-1/2 z-10">
                <svg width="16" height="16" viewBox="0 0 16 16" className="text-slate-600">
                  <path d="M4 8H12M12 8L9 5M12 8L9 11" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none"
                    style={{
                      opacity: inView ? 1 : 0,
                      transition: `opacity 500ms ${(i + 1) * 200 + 300}ms`,
                    }}
                  />
                </svg>
              </div>
    ...
    <div ref={ref} className="mb-5">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-4 text-xs font-mono">
          <span className={isDrawdown ? 'text-emerald-400' : 'text-sky-400'}>{routerValue}{unit}</span>
      ...
      <div className="space-y-1.5">
        <div className="h-2 bg-white/[0.03] rounded-full overflow-hidden">
          <div
            className="h-full rounded-full bg-gradient-to-r from-sky-500 to-sky-400 transition-all duration-1000 ease-out"
            style={{ width: inView ? `${routerWidth}%` : '0%' }}
          />
        </div>
        <div className="h-1.5 bg-white/[0.03] rounded-full overflow-hidden">
          <div
            className="h-full rounded-full bg-slate-600 transition-all duration-1000 ease-out"
            style={{ width: inView ? `${baselineWidth}%` : '0%', transitionDelay: '200ms' }}
          />
        </div>
      </div>
    </div>
...
/* ------------------------------------------------------------------ */
/*  Animated performance metric card                                   */
/* ------------------------------------------------------------------ */
function MetricCard({ value, decimals, suffix, label, sublabel, accent }: {
  ...
  return (
    <div ref={ref} className={`relative p-6 rounded-2xl border ${c.border} ${c.bg} text-center overflow-hidden group hover:scale-[1.02] transition-transform duration-300`}>
      ...
      <p className={`font-mono text-4xl md:text-5xl font-black ${c.text} relative`} style={{ textShadow: accent !== 'white' ? `0 0 30px ${c.glow}` : undefined }}>
        {display}
      </p>
...
/* ------------------------------------------------------------------ */
/*  Simulated growth curve for performance section                     */
/* ------------------------------------------------------------------ */
  ...
  return (
    <div ref={ref} className="relative h-[200px] w-full mt-6 mb-2">
      <svg viewBox="0 0 800 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-full w-full" preserveAspectRatio="none">
        {/* Grid */}
        {[0, 1, 2, 3].map(i => (
          <line key={i} x1="0" y1={50 + i * 50} x2="800" y2={50 + i * 50} stroke="rgba(148,163,184,0.05)" strokeDasharray="4 4" />
        ))}
        <defs>
        ...
        {/* Baseline fill & line */}
        <path d={`${baselinePath} L 800 200 L 0 200 Z`} fill="rgba(148,163,184,0.03)"
          style={{ opacity: inView ? 1 : 0, transition: 'opacity 1s 0.5s' }} />
        <path d={baselinePath} stroke="rgba(148,163,184,0.3)" strokeWidth="1.5" strokeDasharray="4 3"
          style={{ opacity: inView ? 1 : 0, transition: 'opacity 1s 0.5s' }} />
        {/* Router fill & line */}
        <path d={`${routerPath} L 800 200 L 0 200 Z`} fill="url(#perfGrad)"
          style={{ opacity: inView ? 1 : 0, transition: 'opacity 1s 0.3s' }} />
        <path d={routerPath} stroke="#0EA5E9" strokeWidth="2" strokeLinecap="round"
          style={{ opacity: inView ? 1 : 0, transition: 'opacity 1s 0.3s' }} />
        <path d={routerPath} stroke="#0EA5E9" strokeWidth="4" strokeLinecap="round" opacity="0.2" filter="blur(4px)"
          style={{ opacity: inView ? 0.2 : 0, transition: 'opacity 1s 0.3s' }} />
        {/* End dots */}
      ...
      <div className="absolute bottom-0 right-0 flex items-center gap-4 text-[10px] font-mono">
        ...
        <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 bg-slate-500 rounded-full border-b border-dashed border-slate-500" />Single Protocol</span>
      </div>
    </div>
  )
}
...
/* ------------------------------------------------------------------ */
/*  Interactive protocol card                                          */
/* ------------------------------------------------------------------ */
function ProtocolCard({ name, detail, apy, tvl, color }: {
  name: string; detail: string; apy: string; tvl: string; color: string
}) {
  const borderColors: Record<string, string> = {
    sky: 'hover:border-sky-500/30 hover:shadow-[0_0_30px_rgba(14,165,233,0.08)]',
    purple: 'hover:border-purple-500/30 hover:shadow-[0_0_30px_rgba(139,92,246,0.08)]',
...
function DriftTimeline() {
  ...
  return (
    <div ref={ref} className="space-y-0 mb-8">
      {events.map((item, i) => (
        <div key={i} className="flex gap-4 items-start"
          style={{
            opacity: inView ? 1 : 0,
            transform: inView ? 'translateX(0)' : 'translateX(-20px)',
            transition: `all 600ms ${i * 300}ms ease-out`,
          }}
        >
          <div className="flex flex-col items-center gap-0 pt-1.5">
            ...
            {i < 2 && (
              <span className="w-px h-12 bg-gradient-to-b from-white/10 to-transparent" />
            )}
          </div>
          <div className="pb-6">
  ...
  return (
    <nav className="hidden lg:flex fixed left-8 top-1/2 -translate-y-1/2 z-50 flex-col gap-3">
      {sections.map(({ id, number, title }) => {
        ...
        return (
          <a key={id} href={`#${id}`}
            className={['group flex items-center gap-2 transition-all duration-300', isActive ? 'opacity-100' : 'opacity-40 hover:opacity-70'].join(' ')}
          >
            <span className={['font-mono text-[10px] font-bold transition-colors duration-300', isActive ? 'text-sky-400' : 'text-slate-600 group-hover:text-slate-400'].join(' ')}>{number}</span>
            <span className={['text-[10px] tracking-wider uppercase transition-all duration-300 overflow-hidden whitespace-nowrap', isActive ? 'max-w-40 text-white opacity-100' : 'max-w-0 opacity-0 group-hover:max-w-40 group-hover:opacity-100 text-slate-400'].join(' ')}>{title}</span>
            <span className={['h-px transition-all duration-300', isActive ? 'w-6 bg-sky-400' : 'w-2 bg-slate-700 group-hover:w-4 group-hover:bg-slate-500'].join(' ')} />
          </a>
        )
      })}
    </nav>
...
function DotGrid() {
  return (
    <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden" aria-hidden="true">
      ...
      <div className="absolute top-1/3 left-1/2 w-[90vw] h-[60vh] -translate-x-1/2 -translate-y-1/2"
        style={{ background: 'radial-gradient(ellipse, rgba(14,165,233,0.08) 0%, rgba(8,11,17,0) 65%)' }}
      />
    </div>
  )
}
  ...
  return (
    <>
      ...
      <main className="max-w-4xl mx-auto px-6 lg:px-8">
        ...
        {/* ======================================================== */}
        {/*  HERO — Improvement #5: Better hero with dot grid         */}
        {/* ======================================================== */}
        <section className="relative pt-20 pb-16 mb-8 overflow-hidden">
          ...
          <div className="relative z-10">
            <FadeIn>
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm font-mono text-slate-500 mb-12">
                <span>
                  <a href="https://nanuqfi.com" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-white transition-colors">nanuqfi.com</a>
                </span>
                <span className="hidden sm:inline text-slate-700">|</span>
                <span>
                  <a href="https://github.com/nanuqfi" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-white transition-colors">github.com/nanuqfi</a>
                </span>
                <span className="hidden sm:inline text-slate-700">|</span>
                <span>Program: <span className="text-slate-400">2QtJ5k...5z5P</span></span>
              </div>
            </FadeIn>

            {/* Improvement #1: Animated count-up stats */}
          ...
          <div className="pointer-events-none absolute bottom-0 left-0 z-10 h-16 w-full bg-gradient-to-t from-[#080B11] to-transparent" aria-hidden="true" />
        </section>
        ...
        <FadeIn>
          <section id="mechanics" className="scroll-mt-20">
            ...
            <p className="text-slate-400 mb-6">How capital flows through the system:</p>

            {/* Improvement #3: Animated protocol flow diagram */}
            <ProtocolFlowDiagram />

            {/* Three-Layer Validation */}
            <h3 className="text-lg font-semibold text-white mb-6 mt-12">Three-Layer Validation</h3>
            ...
            {/* Improvement #4: Interactive protocol cards */}
        ...
        {/* ======================================================== */}
        {/*  Drift Hack — Improvement #6: Animated timeline           */}
        {/* ======================================================== */}
        <FadeIn>
        ...
        {/* ======================================================== */}
        {/*  Section 4: Performance — Improvements #1, #2, #7         */}
        {/* ======================================================== */}
        <FadeIn>
          <section id="performance" className="scroll-mt-20">
            ...
            {/* Improvement #2: Growth curve chart */}
            <GlassCard className="p-6 mb-8">
              <div className="flex items-center justify-between mb-2">
            ...
            {/* Improvement #7: Comparison bar chart */}
            <GlassCard className="p-6 mb-6">
              ...
              {backtestMetrics.map((m) => (
                <ComparisonBar
                  key={m.label}
                  label={m.label}
                  routerValue={m.router}
                  baselineValue={m.baseline}
                  unit={m.unit}
                  maxValue={Math.max(m.router, m.baseline) * 1.3}
                />
              ))}
            </GlassCard>
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-10">
              ...
              fitted to observed protocol rate ranges. Live performance data from devnet keeper at{' '}
        ...
        {/* ======================================================== */}
        {/*  Section 5: Technical Architecture — Improvement #8       */}
        {/* ======================================================== */}
        <FadeIn>
          <section id="architecture" className="scroll-mt-20">
            ...
            <div className="space-y-2 mb-8">
              {[
                ...
                { pkg: '@nanuqfi/backtest', desc: 'Historical simulation engine', icon: TrendingUp },
              ].map((item) => {
            ...
            {/* Improvement #8: Tech stack badges */}
            <h3 className="text-lg font-semibold text-white mb-4">Tech Stack</h3>
            <div className="flex flex-wrap gap-2 mb-8">
              {techStack.map((tech) => (
                <span key={tech.name} className={`px-3 py-1.5 rounded-lg border text-xs font-medium ${tech.color} transition-all duration-200 hover:scale-105`}>
                  {tech.name}
                </span>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <GlassCard className="p-4 text-center hover:scale-[1.02] transition-transform">
                <p className="font-mono text-lg font-bold text-white">824</p>
                <p className="text-xs text-slate-500">Tests across 3 repos</p>
              </GlassCard>
              <GlassCard className="p-4 text-center hover:scale-[1.02] transition-transform">
                <p className="font-mono text-lg font-bold text-white">CI/CD</p>
                <p className="text-xs text-slate-500">GitHub Actions → GHCR → VPS</p>
              </GlassCard>
            </div>
          </section>
        </FadeIn>
              ...
              {links.map((link) => {
                ...
                return (
                  <a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer"
                    className={[
                      'group relative p-5 rounded-2xl border transition-all duration-300 hover:scale-[1.02] hover:-translate-y-0.5',
                      isLive
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-white flex items-center gap-2">
                          {link.label}
                          {isLive && <span className="w-1.5 h-1.5 rounded-full bg-sky-400 animate-pulse" />}
                        </p>
