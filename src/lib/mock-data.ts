...
// Keeper emits a decision every cycle (~10 min), so the "<24h" bucket needs
// minute-level precision too — otherwise 1h5m, 1h25m, and 1h55m all collapse
// into "1h ago" and the feed loses its ordering signal.
export function formatRelativeTime(timestamp: string): string {
  ...
  if (diffHours < 24) {
    const remMins = diffMins - diffHours * 60
